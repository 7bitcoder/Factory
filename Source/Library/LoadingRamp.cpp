...
namespace sd
{
    ...
    LoadingRamp::LoadingRamp(const LoadingRampData &data) : LoadingRamp(data.id, data.deliveryInterval)
    {
    }
    ...
    std::string LoadingRamp::toString() const
    {
        return std::format("LOADING_RAMP #{}", getId());
    }

    NodeType LoadingRamp::getNodeType() const
    {
        return NodeType::RAMP;
    }

    const LoadingRampData LoadingRamp::getLoadingRampData() const
    {
        return {getId(), getTotalProcesingTime()};
    }

    void LoadingRamp::triggerOperation()
    {
        setProduct(std::move(createProduct()));
    }
