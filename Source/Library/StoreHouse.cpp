...
namespace sd
{
    ...
    std::string StoreHouse::toString() const { return std::format("STOREHOUSE #{}", getId()); }

    NodeType StoreHouse::getNodeType() const { return NodeType::STORE; }

    const StoreHouseData StoreHouse::getStoreHouseData() const { return {getId()}; }
}